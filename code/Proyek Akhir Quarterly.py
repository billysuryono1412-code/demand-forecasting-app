...
db = client["final_project_quarterly"]
...
class SARIMAX_pred:
    ...
    def process_data_item(self, data):
        ...
        seasonal_grid = list(itertools.product(range(0, 2), range(0, 2), range(0, 2), [4])) if is_seasonal == "Data Musiman" else None
        ...
        try:
            ...
            print(f"LinAlgError during retraining: {e} for item: {nama_item}")
            return {
            'Nama Item': nama_item, 
            'ERROR (SARIMAX)': float('inf'),  
            'Model (SARIMAX)': None}
...
class LSTM_pred:
    def __init__(self, product_dfs, time_steps=4):
        self.product_dfs = product_dfs
...
class ForecastGenerator:
    def __init__(self, combined_results, product_dfs, future_steps=4):  # Default to 4 quarters
    ...
    def sarimax_forecast(self, row):
        ...
        start_date = df.index[-1] + pd.DateOffset(months=3)  # Move to next quarter
        date_range = pd.date_range(start=start_date, periods=self.future_steps, freq='QS')
    ...
    def prophet_forecast(self, row):
        ...
        future = model.make_future_dataframe(periods=self.future_steps, freq='QS')
        ...
        forecast['Jumlah Item'] = forecast['yhat']
    ...
    def lstm_forecast(self, row):
        ...
        scaler = MinMaxScaler()
        scaled_data = scaler.fit_transform(df[['Jumlah Item', 'exog1', 'exog2']])
        last_sequence = scaled_data[-4:]  # Quarterly data
        future_preds = []
        ...
        temp = pd.DataFrame({
            ...
            'Model': 'LSTM'
        }, index=pd.date_range(start=df.index[-1] + pd.DateOffset(months=3), periods=self.future_steps, freq='QS'))
...
def forecasting(choice, exog_file, batch_size=5, progress=gr.Progress()):
    ...
    products_df = products_df.groupby([
        pd.Grouper(freq='QS'),  # Use the index with quarterly frequency
        'Nama Item'
    ]).apply(
    ...
    for product in products_df['Nama Item'].unique():
        # Filter the grouped DataFrame for the current product
        temp = products_df[products_df['Nama Item'] == product]
        
        temp.set_index('YearMonth', inplace=True)
        # Step 1: Create a date range covering all quarters between the first and last dates in `YearMonth`
        all_quarters = pd.date_range(start=temp.index.min(), end=products_df['YearMonth'].max(), freq='QS')  # 'QS' for Quarter Start

        # Step 2: Reindex to include all months and fill missing values
        temp = temp.reindex(all_quarters)

        # Step 3: Forward-fill `exog1` and `exog2`, fill `Jumlah Item` with 0
        temp['Nama Item'] = temp['Nama Item'].fillna(product)  # Fill Nama Item
    ...
    product_dfs = [df for df in product_dfs if len(df) >= 8]
    ...
    if exog_file:
        ...
        if len(exog_df.columns) > 0:
            ...
            # Aggregate the exogenous variables to quarterly frequency
            exog_df = exog_df.resample('QS').mean().ffill()
            merged_dfs = [
